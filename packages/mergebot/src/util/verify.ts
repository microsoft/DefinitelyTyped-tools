...
export async function httpLog(context: InvocationContext, headers: Headers, body: any) {
    ...
    context.log(`>>> HTTP Trigger ${context.functionName} [${
                  event}.${body.action
                  }; gh: ${githubId
...
export async function shouldRunRequest(context: InvocationContext, headers: Headers, body: any, canHandleRequest?: (event: string, action: string) => boolean) {
