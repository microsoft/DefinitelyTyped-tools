...
export const suggestTesting = deletedWhenNotPresent("suggest-testing", (tag) => (user: string, testsLink: string) => ({
  tag,
  status: txt`
            |Hey @${user},
            |
...
export const offerSelfMerge = deletedWhenNotPresent(
  "merge-offer",
  (tag) => (user: string, otherOwners: string[], abbrOid: string) => ({
...
export const remindPeopleTheyCanUnblockPR = (
  user: string,
